...
export class ApkTestService {
  ...
  async startTestSession(adminId: string, buildVersion: string, deviceInfo?: any) {
    ...
    const created = await this.prisma.appTestSession.create({
      data: {
        adminId,
        buildVersion,
  ...
  async reportBug(sessionId: string, adminId: string, payload: { title: string; description?: string; screenshotUrl?: string }) {
    const session = await this.prisma.appTestSession.findUnique({ where: { id: sessionId } });
    ...
    await this.prisma.appTestSession.update({ where: { id: sessionId }, data: { bugsFound: bugs as any } });
  ...
  async endTestSession(sessionId: string, adminId: string, testNotes?: string) {
    ...
    if (!session) throw new NotFoundException();
    if (session.adminId !== adminId) throw new BadRequestException("not your session");
    const updated = await this.prisma.appTestSession.update({ where: { id: sessionId }, data: { status: 'COMPLETED', testNotes: testNotes ?? null, endedAt: new Date() } });
    return updated;
  }

  async getTestSessions(adminId: string) {
    return this.prisma.appTestSession.findMany({ where: { adminId }, orderBy: { startedAt: 'desc' } });
  }

  async getLatestBugReports() {
    const since = new Date(Date.now() - 7 * 24 * 3600 * 1000);
    const sessions = await this.prisma.appTestSession.findMany({ where: { startedAt: { gte: since } } });
    const bugs = [] as any[];
